...
class InitialFeatureExtractor(nn.Module):
    ...
    def knn(self, xyz, k):
        ...
        batch_idx = torch.arange(batch_size, device=xyz.device).repeat_interleave(num_points)
    ...
    def forward(self, xyz, feature):
        ...
        f_ck = f_ck.reshape(batch_size, num_points, -1)  # (B, N, k*D)
        TriE = self.trigonometric_embedding(xyz)  # (B, N, 6*D_xyz)
        f_ckw = (f_ck + TriE) * TriE  # (B, N, k*D)
        maxpool_f_ck = torch.max(f_ckw, dim=2)[0]  # (B, N, k*D)
        avgpool_f_ck = torch.mean(f_ckw, dim=2)  # (B, N, k*D)
        aggregated_feature = maxpool_f_ck + avgpool_f_ck  # (B, N, k*D)
